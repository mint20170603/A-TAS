...
$localManifestPath = Join-Path $root "manifest.sha256"
$sourcePath = Join-Path $root "src\A-TAS 4.0.cpp"
$apiUrl = "https://api.github.com/repos/Leonhardqwq/A-TAS/releases/latest"
$tempDir = $null
...
function Read-Manifest([string]$path) {
    ...
    foreach ($line in Get-Content -LiteralPath $path) {
        ...
        if ($relative -match '^(?i:(\.git)(/|$)|settings\.dat$|keybindings\.ini$)') {
...
try {
    ...
    $zipAsset = $release.assets | Where-Object { $_.name -eq "A-TAS.zip" } | Select-Object -First 1
    ...
    if ($null -eq $zipAsset -or $null -eq $manifestAsset) {
        throw "Release assets A-TAS.zip or manifest.sha256 are missing."
    }

    $tempDir = Join-Path ([IO.Path]::GetTempPath()) ("A-TAS-update-" + [guid]::NewGuid().ToString("N"))
    $packageDir = Join-Path $tempDir "package"
    $zipPath = Join-Path $tempDir "A-TAS.zip"
    ...
    foreach ($required in @("A-TAS-Manager.exe", "libavz.dll", "src/A-TAS 4.0.cpp")) {
